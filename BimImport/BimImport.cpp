...
// [참고] 이 함수는 어디에서도 호출되지 않는 죽은 코드(dead code)다. 실제로 폴더에서
// GLTF/GLB 파일 목록을 모으는 데는 아래 collect_gltf_files()가 쓰이고 있으며
// (loadGltf()가 호출), 이 함수는 그와 거의 동일한 기능을 하는 예전 버전으로 보인다.
...
std::vector<std::string> collect_gltf_files(const std::string& folder_path) {
    ...
    if (!fs::exists(input_path) || !fs::is_directory(input_path)) {
        ...
        return gltf_file_paths; /// 빈 vector 반환
    }
...
// bim_folder 안의 모든 GLTF/GLB 파일을 찾아(collect_gltf_files) 각각 chunkbim::GltfMesh
// (include/bim/gltf_mesh.h)로 읽어들여, 하나의 MeshChunk 목록(bimData)으로 합쳐 반환한다.
